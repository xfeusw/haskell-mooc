...
blendColor (Color a b c) (Color x y z) = Color (av a x) (av b y) (av c z)
  where
    av x y = (x + y) `div` 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture a) (Picture b) = Picture k
  where
    k d@(Coord x y) = f (a d) (b d)
...
rectangle x0 y0 w h =  Shape f
  where f (Coord x y) = (x >= x0 && x <= x0 + (w-1)) && y >= y0 && y <= (y0 + (h-1))
...
union a b = Shape f
  where f (Coord x y) = contains a x y  || contains b x y

cut :: Shape -> Shape -> Shape
cut a b = Shape f
  where f (Coord x y) = contains a x y && not (contains b x y)
...
paintSolid color shape (Picture f) = Picture k
  where k a@(Coord x y)  | contains shape x y = color
                         | otherwise  = f a
...
paint (Picture f) shape (Picture h) = Picture k
  where k a@(Coord x y) | contains shape x y = f a
                        | otherwise  = h a
